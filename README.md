...
**Dashboard** — React + Vite + Tailwind + Recharts. Real-time cards, historical charts, alert timeline, logs panel, node comparison, CSV export with date range selector, dark/light theme.
...
To run with demo data (5 nodes, 24h of simulated metrics):
...
- Prometheus metrics: http://localhost:3001/api/v1/metrics/prometheus
...
| GET | `/api/v1/metrics/names/:nodeId` | Distinct metric names |
| GET | `/api/v1/metrics/export?nodeId=&metricName=&from=&to=` | CSV export |
