...
function NodeMetricChart({
  ...
  if (isLoading) {
    return (
      <div className="p-4 text-center text-zinc-400 text-xs font-mono uppercase">
        Loading {nodeName}...
      </div>
    );
  }
  ...
  return (
    <TimeSeriesChart
      title={nodeName}
      subtitle={METRIC_OPTIONS.find((o) => o.value === metric)?.label ?? metric}
      data={chartData}
      series={[{ dataKey: nodeName, color: COLORS[colorIndex % COLORS.length], label: nodeName }]}
    />
  );
}
